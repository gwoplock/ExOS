...
bool Fat12File::readFromDisk( ) {
	...
	uint32_t read = 0;
	while (read < _fileLocSize
			&& _fileLocSize - read
					>= (_device->FSInfo( )->secPerCluster
							* _device->FSInfo( )->bytePerSec)) {
		unsigned int fat_offset = _lastCluster + (_lastCluster / 2); // multiply by 1.5
		unsigned int fat_sector = firstFatSec
				+ (fat_offset / _device->FSInfo( )->bytePerSec);
		...
		if (table_value >= 0xFF8) {
			...
			read += (_device->FSInfo( )->secPerCluster
					* _device->FSInfo( )->bytePerSec);
